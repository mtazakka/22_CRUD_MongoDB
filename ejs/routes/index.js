  ...
  router.get('/', async (req, res) => {
    ...
    if (req.query.dateFilters == 'on') {
      if (req.query.startDate != '' && req.query.toDate != '') {
        var paramsDate = {
        date: {"$gte": req.query.startDate, "$lte":req.query.toDate}
        }
        // paramsDate = JSON.parse(paramsDate)
        // params.push(`date: {$gt: ${req.query.startDate}, $lt: ${req.query.toDate}}`)
      } else if (req.query.startDate) {
        const dated = moment(req.query.startdate).format('DD MMMM YYYY')
        var paramsStartDate = {
          'date' : {$gt: new Date(dated)}
